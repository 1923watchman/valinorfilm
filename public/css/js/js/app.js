document.addEventListener('DOMContentLoaded', () => {
  ...
  if (roomParam) inputRoomId.value = roomParam.trim().toLowerCase();
  ...
  function joinRoom(roomId, username) {
    if (!username || !roomId) { alert('Lütfen kullanıcı adınızı ve oda kodunu girin!'); return; }
    currentRoomId = roomId.toLowerCase().trim();
    currentUsername = username.trim();
    localStorage.setItem('valinor_username', currentUsername);
    headerRoomId.textContent = currentRoomId;
    userBadgeName.textContent = currentUsername;
    roomHeaderBadge.classList.remove('hidden');
  ...
  btnCreateRoom.addEventListener('click', () => joinRoom(inputRoomId.value || generateRoomId(), inputUsername.value));
  btnJoinRoom.addEventListener('click', () => joinRoom(inputRoomId.value, inputUsername.value));
  ...
  btnCopyLink.addEventListener('click', () => {
    navigator.clipboard.writeText(`${window.location.origin}?room=${currentRoomId}`).then(() => {
      ...
      lucide.createIcons();
      setTimeout(() => { btnCopyLink.innerHTML = orig; lucide.createIcons(); }, 2000);
    });
  });
  ...
  btnToggleMic.addEventListener('click', async () => {
    ...
    try {
      await webrtc.toggleCameraAndMic(cameraOn, micOn);
      updateSelfWebcam();
      socket.emit('media-status-update', { micOn, cameraOn });
  ...
  const handleStartScreenShare = async () => {
    if (!isSharingScreen) {
      try {
        ...
        updateScreenShareButton();
        showScreenStream(stream, socket.id, currentUsername);
        socket.emit('screen-share-status', { isSharing: true });
      } catch (err) {}
    } else { webrtc.stopScreenShare(); }
  };

  btnToggleScreen.addEventListener('click', handleStartScreenShare);
  placeholderBtnShare.addEventListener('click', handleStartScreenShare);
  ...
  function updateScreenShareButton() {
    ...
    btnScreenText.textContent = isSharingScreen ? 'Paylaşımı Durdur' : 'Ekran Paylaş';
    statusLiveBadge.classList.toggle('hidden', !isSharingScreen);
  }
  ...
  function showScreenStream(stream, sharerId, sharerName = 'Yayıncı') {
    ...
    videoOverlayHeader.classList.remove('hidden');
    volumeControlWrapper.classList.remove('hidden');
    const user = roomUsersMap.get(sharerId);
    screenSharerName.textContent = user ? user.username : sharerName;
  }
  ...
  function hideScreenStream() {
    ...
    videoOverlayHeader.classList.add('hidden');
    volumeControlWrapper.classList.add('hidden');
    statusLiveBadge.classList.add('hidden');
  }

  volumeSlider.addEventListener('input', (e) => screenVideo.volume = e.target.value);
  btnMuteVolume.addEventListener('click', () => {
    screenVideo.volume = screenVideo.volume > 0 ? 0 : 1;
    volumeSlider.value = screenVideo.volume;
  ...
  function updateSelfWebcam() {
    if (!cameraOn && !micOn) { removeWebcam(socket.id); return; }
    let card = document.getElementById(`webcam-card-${socket.id}`);
    if (!card) {
      card = document.createElement('div');
      card.id = `webcam-card-${socket.id}`;
      card.className = 'webcam-card valinor-glow-purple';
      card.innerHTML = `<video id="webcam-video-${socket.id}" autoplay playsinline muted></video><div class="user-label"><span class="truncate font-semibold text-amber-300">Sen (${currentUsername})</span><div class="flex items-center gap-1"><i id="mic-status-${socket.id}" data-lucide="${micOn ? 'mic' : 'mic-off'}" class="w-3 h-3 ${micOn ? 'text-emerald-400' : 'text-rose-400'}"></i></div></div>`;
      webcamGrid.appendChild(card);
      lucide.createIcons();
    }
    const videoElem = document.getElementById(`webcam-video-${socket.id}`);
    if (webrtc.localCamStream && cameraOn) videoElem.srcObject = webrtc.localCamStream;
  ...
  function addOrUpdateWebcam(peerSocketId, stream) {
    ...
    document.getElementById(`webcam-video-${peerSocketId}`).srcObject = stream;
  }
  ...
  socket.on('room-users', (users) => {
    roomUsersMap.clear();
    users.forEach(u => { roomUsersMap.set(u.socketId, u); webrtc.initiateOffer(u.socketId); });
  ...
  socket.on('user-left', ({ socketId, username }) => {
    ...
    webrtc.closePeerConnection(socketId);
    renderUsersList();
  ...
  socket.on('media-status-changed', ({ socketId, micOn: pMic, cameraOn: pCam }) => {
    ...
    if (user) {
      ...
      if (micIcon) { micIcon.setAttribute('data-lucide', pMic ? 'mic' : 'mic-off'); micIcon.className = `w-3 h-3 ${pMic ? 'text-emerald-400' : 'text-rose-400'}`; lucide.createIcons(); }
      renderUsersList();
    }
  });
  ...
  function addChatMessage({ senderId, username, message, time }) {
    ...
    if (sidebar.classList.contains('hidden') || tabContentChat.classList.contains('hidden')) unreadChatDot.classList.remove('hidden');
  }
  ...
  function renderUsersList() {
    usersList.innerHTML = '';
    userCount.textContent = roomUsersMap.size;
  ...
  tabBtnChat.addEventListener('click', () => {
    ...
    tabBtnUsers.className = 'flex-1 py-3 px-4 text-xs font-bold tracking-wider uppercase flex items-center justify-center gap-2 border-b-2 border-transparent text-slate-400 hover:text-slate-200 transition';
  ...
  tabBtnUsers.addEventListener('click', () => {
    ...
    tabBtnChat.className = 'flex-1 py-3 px-4 text-xs font-bold tracking-wider uppercase flex items-center justify-center gap-2 border-b-2 border-transparent text-slate-400 hover:text-slate-200 transition';
